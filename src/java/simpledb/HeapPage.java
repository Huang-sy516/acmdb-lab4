...
public class HeapPage implements Page {
    ...
    */
    ...
    public void deleteTuple(Tuple t) throws DbException {
        ...
        if (t.getRecordId().tupleno() >= this.numSlots) {  // 判断tuple的tupleNo是否超过Page的限制
            throw new DbException("tuple is not on this page");
        }
        if (!this.td.equals(t.getTupleDesc())) {  // tupledesc不匹配，因此不在这个page上
            throw new DbException("tuple is not on this page");
        }
        if (tuples[t.getRecordId().tupleno()] == null) {  // 判断该page的相应位置上有没有tuple
            throw new DbException("tuple is not on this page");
        }
        if (!isSlotUsed(t.getRecordId().tupleno())) {  // 判断该slot是否为空
            throw new DbException("slot is empty");
        }
        if (!t.getRecordId().getPageId().equals(this.pid)) {  // 判断相应位置上的tuple是否相同
            throw new DbException("tuple is not on this page");
        }
        this.tuples[t.getRecordId().tupleno()] = null;  // 删除该page存储的tuple
    ...
    public boolean isSlotUsed(int i) {
        // some code goes here
        // check whether the specific bit of the bitmap is 1 
        int bytes = i / 8;
        int bits = i % 8;
