...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// some code goes here
        // Begin recursive descent through the B+ tree
		if (pid.pgcateg() == BTreePageId.LEAF) {
			// When the leaf page is reached, return it with the specified permissions
			return (BTreeLeafPage) this.getPage(tid, dirtypages, pid, perm);
		} else {
			// Fetch the current internal page at the specified read-only permission
			...
			if (f == null) {
				// Target the first child page if 'f' is null, to get the left-most leaf
				return findLeafPage(tid, dirtypages, internalPage.getChildId(0), perm, null);
			}
			
			// Iterate over the entries to find the correct child based on 'f'
			while (entries.hasNext()) {
				BTreeEntry entry = entries.next();
				if (entry.getKey().compare(Op.GREATER_THAN_OR_EQ, f)) {
					// Follow the left child if the entry key matches or is greater than 'f'
					return findLeafPage(tid, dirtypages, entry.getLeftChild(), perm, f);
				}
			}
			
			// If no matching entries, proceed with the last child
			return findLeafPage(tid, dirtypages, internalPage.getChildId(internalPage.getNumEntries()), perm, f);
		}
	}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		// Update parent pointers and propagate changes upwards
		Field middleKey = newLeaf.getTuple(0).getField(page.keyField);
		BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), middleKey);
		parent.insertEntry(new BTreeEntry(middleKey, page.getId(), newLeaf.getId()));
		...
		// Return the correct leaf page for the field
		return field.compare(Op.GREATER_THAN_OR_EQ, middleKey) ? newLeaf : page;
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
			BTreeInternalPage page, Field field) 
					throws DbException, IOException, TransactionAbortedException {
		// some code goes here
		// Prepare a new internal page
		BTreeInternalPage newInternal = (BTreeInternalPage) getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);
		// Reverse iterator to move half of the entries to the new page
		Iterator<BTreeEntry> reverseIterator = page.reverseIterator();
		int middleIndex = page.getNumEntries() / 2;
		while (newInternal.getNumEntries() < middleIndex) {
			BTreeEntry currentEntry = reverseIterator.next();
			page.deleteKeyAndRightChild(currentEntry);
			newInternal.insertEntry(currentEntry);
		}
	
		// Handle the middle key to push up
		...
		// Return the appropriate page based on the field comparison
		return field.compare(Op.GREATER_THAN_OR_EQ, middleEntry.getKey()) ? newInternal : page;
	}
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		// Calculate the number of tuples to move to balance both pages
		int neededTuples = (page.getNumTuples() + sibling.getNumTuples()) / 2 - page.getNumTuples();
		// Use the appropriate iterator based on the sibling's position to maintain order
		Iterator<Tuple> iter = isRightSibling ? sibling.iterator() : sibling.reverseIterator();
	
		// Move tuples from sibling to underfilled page
		for (int i = 0; i < neededTuples; i++) {
			Tuple tuple = iter.next();
			sibling.deleteTuple(tuple);
			page.insertTuple(tuple);
		}
	
		// Update the parent's key to reflect the new smallest key in the right sibling after redistribution
		entry.setKey(iter.next().getField(page.keyField));
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// some code goes here
		// Calculate how many entries to move to make both pages at least half full
		int entriesToRedistribute = (page.getNumEntries() + leftSibling.getNumEntries()) / 2 - page.getNumEntries();
		// Reverse iterator to start moving entries from the end of the left sibling
		Iterator<BTreeEntry> entryIterator = leftSibling.reverseIterator();
		BTreeEntry transferredEntry = entryIterator.next();
	
		// Insert a new entry into the page, which involves pulling down the parent key
		BTreeEntry midEntry = new BTreeEntry(parentEntry.getKey(), transferredEntry.getRightChild(), page.iterator().next().getLeftChild());
		page.insertEntry(midEntry);
		entriesToRedistribute--;
	
		// Continue moving entries until the pages are balanced
		for (int i = 0; i < entriesToRedistribute; i++) {
			leftSibling.deleteKeyAndRightChild(transferredEntry);
			page.insertEntry(transferredEntry);
			transferredEntry = entryIterator.next();
		}
	
		// Update the parent key to the last moved key
		leftSibling.deleteKeyAndRightChild(transferredEntry);
		parentEntry.setKey(transferredEntry.getKey());
		parent.updateEntry(parentEntry);
	
		// Update dirty pages and parent pointers
		dirtypages.put(page.getId(), page);
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// Determine the number of entries needed to balance the two pages
		int entriesToRedistribute = (page.getNumEntries() + rightSibling.getNumEntries()) / 2 - page.getNumEntries();
		// Use a regular iterator for the right sibling to move entries to the left
		Iterator<BTreeEntry> entryIterator = rightSibling.iterator();
		BTreeEntry transferredEntry = entryIterator.next();
	
		// Create a new entry by pulling down the parent key between the two sibling entries
		BTreeEntry midEntry = new BTreeEntry(
				parentEntry.getKey(),
				page.reverseIterator().next().getRightChild(),
				transferredEntry.getLeftChild()
		);
		page.insertEntry(midEntry);
		entriesToRedistribute--;
	
		// Continue moving entries until balanced
		for (int i = 0; i < entriesToRedistribute; i++) {
			rightSibling.deleteKeyAndLeftChild(transferredEntry);
			page.insertEntry(transferredEntry);
			transferredEntry = entryIterator.next();
		}
	
		// Update the parent key to the new first key of the right sibling
		rightSibling.deleteKeyAndLeftChild(transferredEntry);
		parentEntry.setKey(transferredEntry.getKey());
		parent.updateEntry(parentEntry);
	
		// Update dirty pages and ensure correct parent pointers
		dirtypages.put(page.getId(), page);
		dirtypages.put(rightSibling.getId(), rightSibling);
		dirtypages.put(parent.getId(), parent);
		updateParentPointers(tid, dirtypages, page);
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// Transfer all tuples from the right to the left leaf page.
		Iterator<Tuple> tuplesToMove = rightPage.iterator();
		while (tuplesToMove.hasNext()) {
			Tuple tuple = tuplesToMove.next();
			rightPage.deleteTuple(tuple);
			leftPage.insertTuple(tuple);
		}
	
		// Correctly update the sibling pointers after the merge.
		if (rightPage.getRightSiblingId() != null) {
			BTreeLeafPage newRightSibling = (BTreeLeafPage) this.getPage(tid, dirtypages, rightPage.getRightSiblingId(), Permissions.READ_WRITE);
			leftPage.setRightSiblingId(newRightSibling.getId());
		...
		// Free the right page and update parent structure.
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// Delete the entry in the parent corresponding to the two pages that are merging -
		// deleteParentEntry() will be useful here
		// Create a new entry to connect the last child of the left page to the first child of the right page.
		BTreeEntry connectingEntry = new BTreeEntry(parentEntry.getKey(),
		leftPage.reverseIterator().next().getRightChild(),
		rightPage.iterator().next().getLeftChild());
		leftPage.insertEntry(connectingEntry);

		// Move all entries from the right internal page to the left.
		Iterator<BTreeEntry> entriesToMove = rightPage.iterator();
		while (entriesToMove.hasNext()) {
		BTreeEntry entry = entriesToMove.next();
		rightPage.deleteKeyAndLeftChild(entry);
		leftPage.insertEntry(entry);
		}

		// Update the parent pointers and free the right page.
